...
import { Bell, CreditCard, ShieldCheck, TrendingUp, AlertTriangle, Plus, History } from "lucide-react";
import { useEffect, useState } from "react";
import { PaymentProfile, paymentsService, Utility, Bill } from "@/lib/paymentsService";
            ...
            <div className="bg-primary/5 px-4 pt-6 pb-6 rounded-b-3xl">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p className="text-muted-foreground text-sm">Track bills & avoid scams</p>
                    </div>
                    <div className="flex gap-2">
                        <Button variant="outline" size="icon" className="rounded-full bg-background border-0 shadow-sm">
                ...
                {/* Amount Due Card */}
                <Card className="p-5 border-0 shadow-lg bg-gradient-to-br from-primary to-indigo-600 text-white relative overflow-hidden">
                    <div className="relative z-10">
                        <p className="text-white/80 text-sm font-medium mb-1">Total Due</p>
                        <h2 className="text-4xl font-bold mb-4">₹{profile?.totalDue.toLocaleString()}</h2>

                        <div className="flex gap-2">
                            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium backdrop-blur-md ${(profile?.safetyScore || 0) > 80 ? 'bg-green-500/20 text-green-100' : 'bg-yellow-500/20 text-yellow-100'
                                }`}>
                    ...
                    <>
                        ...
                        <div className="space-y-4">
                            {profile?.utilities.map(utility => {
                                const latestBill = profile.bills.find(b => b.utilityId === utility.id); // In real app, explicit link
                        ...
                        <div className="mt-8 mb-4">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <Button variant="ghost" size="sm" className="h-8 text-muted-foreground">View All</Button>
                            </div>
                            <Card className="p-4 border bg-card/50">
                                <p className="text-sm text-center text-muted-foreground py-2">No recent transactions</p>
                            </Card>
                        </div>
                    </>
                ) : (
                    <div className="space-y-4">
                        ...
                        <Card className="p-4 border-yellow-200 bg-yellow-50">
                            <div className="flex gap-3">
                                ...
                                <div>
                                    <h3 className="font-semibold text-yellow-900">Payment Safety Tips</h3>
                                    <p className="text-sm text-yellow-800/80 mt-1">
                                        Always verify the URL before paying. Official utility websites usually end in .gov.in or .org.
                                    </p>
                                </div>
                            </div>
                        </Card>
