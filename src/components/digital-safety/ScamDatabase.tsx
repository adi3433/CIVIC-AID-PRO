...
export function ScamDatabase() {
  ...
  const filteredScams = reportedScams.filter(
    (scam) =>
  ...
  const getCategoryColor = (category: ScamCategory) => {
    // Using semantic badge variants for different categories
    const colorMap: Record<string, "destructive" | "secondary" | "outline"> = {
      phishing: "destructive",
      fake_govt_scheme: "secondary",
      job_scam: "secondary",
      payment_scam: "destructive",
      loan_scam: "secondary",
      impersonation: "destructive",
      lottery_scam: "secondary",
      investment_scam: "destructive",
    };
    return colorMap[category] || "outline";
  };
      ...
      <Card variant="default" className="overflow-hidden">
        <div className="p-4 bg-muted/50 border-b">
          ...
          <div className="flex items-center gap-2">
            <Badge variant={getCategoryColor(selectedScam.type)}>
              {getCategoryDisplayName(selectedScam.type)}
            </Badge>
            <Badge variant="outline" className="bg-muted">
              {getChannelIcon(selectedScam.channel)}{" "}
              {selectedScam.channel.replace("_", " ")}
            </Badge>
          </div>
        </div>
        ...
        <div className="p-4 space-y-4">
          {/* Scam Content */}
          <Card variant="destructive" className="bg-destructive/10">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="p-3 bg-muted/50 rounded-lg">
              ...
              <Badge
                variant={
                  selectedScam.status === "verified" ? "default" : "secondary"
                }
              >
                {selectedScam.status}
              </Badge>
          ...
          {selectedScam.amountLost && (
            <Card
              variant="destructive"
              className="bg-destructive/10 flex items-center justify-between"
            >
              <span className="text-sm text-foreground">Reported Loss</span>
              <span className="font-bold text-destructive">
                ₹{selectedScam.amountLost.toLocaleString()}
              </span>
            </Card>
          )}
          ...
          <Card
            variant="warning"
            className="bg-warning/10 flex items-start gap-2"
          >
    ...
    <div className="space-y-4">
      ...
      <Card variant="warning">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-lg text-foreground">Scam Database</h2>
      ...
      <Card variant="elevated">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Card variant="primary" className="bg-primary/10 p-3 text-center">
            <p className="text-2xl font-bold text-primary">
              {stats.totalReports.toLocaleString()}
            </p>
            <p className="text-xs text-muted-foreground">Total Reports</p>
          </Card>
          <Card variant="success" className="bg-success/10 p-3 text-center">
            <p className="text-2xl font-bold text-success">
              ₹{(stats.totalMoneySaved / 10000000).toFixed(1)}Cr
            </p>
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge variant={getCategoryColor(scam.type)}>
                ...
                <div key={trend.category} className="p-3 border rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={getCategoryColor(trend.category)}>
                    ...
                    <div className="text-right">
                      ...
                      <p
                        className={`text-xs ${trend.percentageChange > 0 ? "text-red-500" : "text-green-500"}`}
                      >
