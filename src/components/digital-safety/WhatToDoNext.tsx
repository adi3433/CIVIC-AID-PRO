...
import { getGuidanceForScamType, PostScamGuidance, GuidanceStep } from "@/lib/redFlags";

type ScamType = "payment_scam" | "phishing" | "job_scam" | "impersonation" | "lottery_scam" | "investment_scam";

const scamTypeOptions: { value: ScamType; label: string; icon: React.ReactNode }[] = [
    { value: "payment_scam", label: "UPI/Payment Fraud", icon: <CreditCard className="w-4 h-4" /> },
    { value: "phishing", label: "Bank/KYC Phishing", icon: <ShieldAlert className="w-4 h-4" /> },
    { value: "job_scam", label: "Job/Employment Scam", icon: <FileText className="w-4 h-4" /> },
    { value: "impersonation", label: "Police/Authority Call", icon: <Phone className="w-4 h-4" /> },
    { value: "lottery_scam", label: "Lottery/Prize Scam", icon: <AlertTriangle className="w-4 h-4" /> },
    { value: "investment_scam", label: "Investment Fraud", icon: <AlertTriangle className="w-4 h-4" /> },
];
                ...
                <div className="space-y-2">
                    ...
                    >
                        <button
                            ...
                            className={`mt-0.5 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${isCompleted
                        ...
                        <div className="pl-9 space-y-3 pt-2">
                            ...
                            {step.contactInfo && (
                                <div className="flex items-center gap-2 p-2 bg-muted/50 rounded-lg">
                                    ...
                                    <span className="text-sm font-medium">{step.contactInfo}</span>
            ...
            <div className="p-4 bg-gradient-to-r from-red-500 to-orange-500 text-white">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-white/80">Get step-by-step recovery guidance</p>
                    </div>
                </div>
            </div>
                    ...
                    <>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {scamTypeOptions.map((option) => (
                                    <button
                                        ...
                                        className={`p-3 rounded-lg border text-left transition-all ${scamType === option.value
                                                ? "border-primary bg-primary/10"
                                                : "border-muted hover:border-primary/50"
                                            }`}
                                    >
                                        <div className="flex items-center gap-2">
                                            <span className={scamType === option.value ? "text-primary" : "text-muted-foreground"}>
                                                {option.icon}
                                            </span>
                                            <span className={`text-sm font-medium ${scamType === option.value ? "text-foreground" : "text-muted-foreground"
                                                }`}>
                                                {option.label}
                                            </span>
                                        </div>
                        ...
                        <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            <p className="text-xs text-blue-800">
                                <strong>Don't panic!</strong> Many scams can be recovered if you act quickly.
                                We'll guide you through exactly what to do.
                            </p>
                        </div>
                    </>
                ) : guidance ? (
                    <>
                        ...
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <span className="text-sm font-medium text-primary">
                                {completedSteps.length} / {guidance.immediateSteps.length + guidance.followUpSteps.length} steps
                            </span>
                        </div>
                        ...
                        {/* Prevention Tips */}
                        <div className="space-y-2 pt-4 border-t">
                            <h3 className="font-semibold text-foreground">🛡️ Prevent Future Scams</h3>
